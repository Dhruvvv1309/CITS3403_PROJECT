...
class Message(db.Model):
    ...
    def to_dict(self):
        # Convert UTC timestamp to Perth time (AWST = UTC+8)
        perth_tz   = timezone(timedelta(hours=8))
        perth_time = self.timestamp.replace(tzinfo=timezone.utc).astimezone(perth_tz)
        return {
            'id':          self.id,
            'sender_id':   self.sender_id,
            'receiver_id': self.receiver_id,
            'body':        self.body,
            'timestamp':   perth_time.strftime('%H:%M'),
            'date':        perth_time.strftime('%d %b %Y'),
            'read':        self.read,
        }
